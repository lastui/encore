...
impl Parser {
    ...
    pub fn parse_class_declaration(&mut self) -> ParseResult<ClassDeclaration> {
        ...
        let super_class = self.match_token(&Token::Extends)
    ...
    pub fn parse_class_expression(&mut self) -> ParseResult<Expression> {
        ...
        let id = matches!(self.peek(), Some(Token::Identifier(_)))
    ...
    pub fn parse_class_member(&mut self) -> ParseResult<ClassMember> {
        ...
        if is_static && self.check(&Token::LeftBrace) {
        ...
        // Class field
        let value = self.match_token(&Token::Equal)
            .then(|| self.parse_expression())
            .transpose()?;

        self.consume(&Token::Semicolon, "Expected ';' after class field")?;
    ...
    pub fn parse_static_block(&mut self) -> ParseResult<ClassMember> {
        self.consume(&Token::LeftBrace, "Expected '{' after 'static'")?;
        ...
        self.consume(&Token::RightBrace, "Expected '}' after static block")?;
